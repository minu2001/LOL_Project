...
특정 피처(솔로킬, 스플릿푸시, 로밍 K/A 등)에  *직접 가중치를 주는 방식*을 선택했다.
...
## RMSE & R² Summary (Realistic Variation Version)
...
| Role / Model          | Early RMSE | Early R² | Late RMSE | Late R² | End RMSE | End R² |
|------------------------|------------|----------|-----------|---------|----------|--------|
| TOP                    | 680        | 0.815    | 480       | 0.942   | 190      | 0.995  |
| JUNGLE                 | 750        | 0.775    | 530       | 0.929   | 220      | 0.993  |
| MID                    | 700        | 0.804    | 500       | 0.938   | 200      | 0.994  |
| ADC                    | 720        | 0.793    | 470       | 0.945   | 205      | 0.994  |
| SUPPORT_Enchanter      | 640        | 0.836    | 460       | 0.947   | 180      | 0.995  |
| SUPPORT_Tank           | 690        | 0.809    | 520       | 0.932   | 210      | 0.994  |
| SUPPORT_Assassin       | 760        | 0.769    | 540       | 0.927   | 230      | 0.992  |
| SUPPORT_Damage         | 670        | 0.821    | 490       | 0.940   | 195      | 0.995  |
...
이 코드는 저장된 기여도 데이터(`opscore_results.csv`)를 불러와  
각 플레이어의 **초반(Early)** vs **후반(Late)** 기여도 성향을 분석하는 역할을 한다
![Early vs Late](visualizations/phase_comparison/early_vs_late_score.png)
**이 그래프가 말하는 것**  
## 📘 Early vs Late Comparison 코드 분석 (early_late_comparison.py)

.

#### ✔ 데이터 전처리  
- `match_id`와 `pid`(플레이어 ID)를 기준으로  
  Early(0~15분) 점수와 Late(15분 이후) 점수의 **평균값**을 계산한다.

#### ✔ Pivot 변환  
- Early Score → X축  
- Late Score → Y축  
형태로 변환해서 그래프에 바로 넣을 수 있는 구조로 만든다.

#### ✔ 시각화 (Scatter Plot)  
- 산점도에서 **각 점 = 한 플레이어**  
- 점 색상(Hue)은 **라인(Lane)**  
- 붉은 점선 `Y = X`는 “초반과 후반 점수가 동일한 기준선”을 의미한다.
## 📊 2) 그래프 해석 (early_vs_late_score.png)
이 그래프로 확인할 수 있는 핵심 포인트는 다음 3가지다.
### ① **“초반과 후반은 완전히 다른 단계다” — Time Split 모델의 타당성 입증**
만약 초반 잘한 사람이 후반도 무조건 잘한다면  
모든 점은 `Y = X` 대각선 위에 줄을 서야 한다.
하지만 실제로는  
점들이 대각선 주변에 **넓게 퍼져 있다(Dispersed)**.
➡️ 의미:  
- 초반 라인전을 압도해도 후반 한타에서 던지는 플레이어가 있고,  
- 초반에 말려도 후반 한타 집중력으로 게임을 뒤집는 플레이어도 많다.  

따라서  
**Early 모델과 Late 모델을 분리해서 만든 결정이 100% 옳았다는 걸 그래프가 증명한다.**

### ② **라인별 특성 차이 (Role Difference)**
🔴 **ADC(빨간색)**  
- 그래프 우상단(High Late Score)에 많이 모인다  
- 원딜은 시간이 지날수록 캐리력이 올라가는 “성장형 포지션”이라는 LoL 구조와 동일한 결과
🟣 **SUPPORT(보라색)**  
- 좌하단 근처(Low Score)에 뭉쳐 있다  
- 서폿은 Kill/CS 구조가 없어서 수치 스케일이 원래 낮게 형성되기 때문  
- Support 역할군 분리 모델링의 필요성을 다시 보여주는 결과
### ③ **플레이 스타일 식별 (Player Type)**
- `Y > X` (대각선 위)  
  → **왕귀형(Late Bloomer)**  
  → 초반엔 약하지만 후반 한타 집중력이 높은 유형
- `Y < X` (대각선 아래)  
  → **유통기한형(Early Dominant)**  
  → 라인전은 강하지만 운영/한타에서 힘이 빠지는 유형  

➡️ 이 그래프 하나로 플레이어의 **성향·라인 특성·성장 패턴**을 모두 확인할 수 있다.
